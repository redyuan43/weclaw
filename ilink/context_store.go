...
// LoadContextTokens returns a copy of all cached context tokens for a bot.
func LoadContextTokens(botID string) (map[string]string, error) {
	if botID == "" {
		return nil, nil
	}
	...
	var data contextTokenData
	if err := json.Unmarshal(raw, &data); err != nil {
		return nil, fmt.Errorf("parse context tokens: %w", err)
	}
	tokens := make(map[string]string, len(data.Tokens))
	for userID, token := range data.Tokens {
		if userID != "" && token != "" {
			tokens[userID] = token
		}
	}
...
// ClearContextToken removes the cached iLink context token for one user.
func ClearContextToken(botID, userID string) error {
	if botID == "" || userID == "" {
		return nil
	}
